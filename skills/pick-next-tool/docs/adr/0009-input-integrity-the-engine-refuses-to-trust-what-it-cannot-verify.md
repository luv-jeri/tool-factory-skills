...
4. **`thin_site_proof` must carry evidence, and the proof page must itself be thin.** It is honored only when accompanied by the ranking URL + that page's DR + the keyword it ranks for; a bare asserted boolean is treated as unproven (and flagged). **Crucially the proof page's DR must itself be thin (`≤ 40`)** — an evidenced page with a high DR (e.g. 91) is not a thin site but a strong incumbent and must not floor winnability. (This exact seam — a DR-91 proof flipping a Gate-B DROP into a committable OK at opp 87 — was caught by the post-hardening adversarial verification pass and is now locked by a golden-bad fixture.) `thin_site_proof_dr` is range-validated to a finite 0–100, all numeric inputs must be finite (NaN/Inf raise rather than bypass the `<` gates), and the floor is additionally capped at 3 when `kd_head > 80`.
